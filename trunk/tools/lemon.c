...
** function.
*/
PRIVATE char *file_makename(const char *filename, const char *suffix)
{
  char *name;
  char *cp;
...
PRIVATE FILE *file_open(
  ...
  lemp->outname = file_makename(lemp->filename, suffix);
...
void ReportTable(
  ...
  if( in==0 ) return;
  if ( user_outputname ){
    // XXX - This should be moved to file_open
    out = fopen(user_outputname, "wb");
    if ( lemp->outname ) free(lemp->outname);
    lemp->outname = strdup(user_outputname);
  } else {
  ...
  if( mhflag ){
    char *name = file_makename(lemp->outname, ".h");
